...
use crate::modules::signup::interfaces::user_interfaces::{NewUserInterface, UserRequest, UserResponse};
...
impl SignUpService {
    ...
    pub async fn signup_user(&self, user: UserRequest) -> Result<UserResponse, Json<ErrorResponse>> {
        // Validate user data
        if user.password != user.confirm_password {
            return Err(Json(ErrorResponse::new("Passwords do not match")));
        }

        match self.user_repository.exists_by_username(&user.username).await {
        ...
        // Hash the password
        let hashed_password = Self::hash_password(&user.password).await
            .map_err(|e| Json(ErrorResponse::new(e)))?;

        // Save user to database
        let created_user = self.user_repository.create_user(&NewUserInterface{
